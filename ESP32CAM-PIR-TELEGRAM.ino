...
#define BOTtoken "5727779204:AAGyUd5GV49QQh8IXsYviUazSv_alarfkYU"  // your Bot Token (Get from Botfather)
...
static void setupinterrupts() {
  ...
  pinMode(PIRpin, INPUT) ; //INPUT_PULLDOWN);
  ...
  esp_err_t err = gpio_isr_handler_add(static_cast<gpio_num_t>(PIRpin), &PIR_ISR, nullptr);

  if (err != ESP_OK) {
    Serial.printf("gpio_isr_handler_add failed (%x)", err);
  }
  gpio_set_intr_type(static_cast<gpio_num_t>(PIRpin), GPIO_INTR_POSEDGE);
...
static void IRAM_ATTR PIR_ISR(void* arg) {
  ...
  if (digitalRead(PIRpin) == HIGH) {
    // Serial.print("PIR Interupt>> "); Serial.println(digitalRead(PIRpin));
